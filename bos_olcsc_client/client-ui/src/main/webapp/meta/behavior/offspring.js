// ::children-of-the-dom (CSS class auto-creation)
// .first-child
// .last-child
// .odd-child
// .even-child
...
var offspring = {
	...
	traverseChildrenQueue: [], // currently unused
	...
	onload: function() {
		...
		// jslog.info("Exec time: " + (endTime.getTime() - startTime.getTime()) + "ms");
	},
	...
	*/
		...
		switch (shouldRemoveOldOffspringClassesFirst)
		{
			case true: // shouldRemoveOldOffspringClassesFirst is true
					...
					for (var i = 0; i < lastIndex; i++)
					{
						...
						this.addOffspringClassNames(currentElement, i, false);
					...
					this.addOffspringClassNames(lastElement, lastIndex, true);
	...
	addOffspringClassNames: function(element, index, isLastElement) {
	...
	// Borrowed from Prototype.lite

	addClassName: function(element, className) {
		if (!element) return;
		
		// If the current class is empty, just do a simple assignment
	...
	removeClassName: function(element, className) {
		...
		var classHasChanged = false;
		var newClassName = new StringBuffer();
		
		var a = element.className.split(' ');
    	for (var i = 0, len=a.length; i < len; i++) {
			var currentClassName = a[i];
      		if (currentClassName == className) {
				classHasChanged = true;
				// don't append that one to the new class
      		}
			else {
				// The "if (i)" bit checks if i > 0
				if (i) newClassName.append(' ');
        		newClassName.append( currentClassName );

			}
    	}
		if (!classHasChanged) return;
   		element.className = newClassName.toString();
	},
...
/* The StringBuffer object can be used to concatenate strings
	more quickly than the plus-operator ("+").
	...
	Here's an example usage:
		...
		var buf = new StringBuffer();
		 ...
		 alert(myBuf.toString());

	This page has more info on StringBuffers:
